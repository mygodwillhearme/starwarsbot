...
class Die(object):
    ...
    def roll(self):
        ...
        """
        if self.die_type[0] == "d" and self.die_type[1] != "":
...
def roll_string(string):
    ...
    checkList = re.findall(r"([A-Za-z])(\d+)?", string)
    
    for chk in checkList:
            if not chk[0] in list(dice_values.DIE_OPTIONS.keys()) + ["D"]:
                return "Invalid die type supplied. Valid dice are: " + ", ".join(list(dice_values.DIE_OPTIONS.keys()) + ["D"])
...
def display_results(results):
    ...
    if str(results) == "Invalid die type supplied. Valid dice are: " + ", ".join(list(dice_values.DIE_OPTIONS.keys()) + ["D"]):
        return str(results)
    ...
    if results['success'] < 1:
        ...
        output.append("The roll failed with {failure} failure.\n".format(**results))
    else:
       output.append("The roll succeeded with {success} success!\n".format(**results))
        # print("The roll succeeded with {success} success!".format(**results))
        

    if results['advantage'] > 0:
        output.append("The roll generated {advantage} advantage!\n".format(**results))
